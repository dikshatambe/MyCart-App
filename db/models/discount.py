...
class Discount:
    def __init__(self, db_conn, discount_id, name=None, amount=None, discount=None, discount_type=None,quantity=None):
    ...
    def update_category(self, id = None):
        cur = self.db_conn.cursor()
        cur.execute('UPDATE self.table_name SET discount_id=self.id,name=self.name, amount=self.amount, discount=self.discount,discount_type=self.type,quantity=self.quantity where product_id=id;')
        data = cur.fetchall()
        if len(data) >0:
            usr = user_data_to_dict(data[0])
        else:
    ...
    def delete_category(self, id = None):
        cur = self.db_conn.cursor()
        cur.execute('DELETE self.table_name where discount_id=id;')
        cur.close()
        self.db_conn.commit()  
